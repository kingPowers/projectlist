...
define('APPKEY','ZEPodb0DonAx24J98IvOj4');
define('APPID','1R9662zWVt8BuXefR1Z3W6');
define('MASTERSECRET','v9biLWRVue92aQJGNHflr4');



//define('CID','d871ef83a56f3cafc20d516408e0acf1');
//define('DEVICETOKEN','');
//define('Alias','请输入别名');
...
function IGtLinkTemplateDemo(){
    ...
    $template ->set_appId(APPID);//应用appid
    $template ->set_appkey(APPKEY);//应用appkey
    $template ->set_title("我是通知标题");//通知栏标题
    $template ->set_text("你好，我是通知的内容，请在这里给我说明");//通知栏内容
    $template ->set_logo("");//通知栏logo
    $template ->set_isRing(true);//是否响铃
    $template ->set_isVibrate(true);//是否震动
    $template ->set_isClearable(true);//通知栏是否可清除
    $template ->set_url("http://wx1.lingqianzaixian.com/member/system");//打开连接地址
    //$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
...
function IGtTransmissionTemplateDemo(){
    ...
    $template->set_appkey(APPKEY);//应用appkey
    $template->set_transmissionType(1);//透传消息类型
    $template->set_transmissionContent("你好帅哥，我要在通知栏中显示");//透传内容
    //$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
    //APN简单推送
//        $template = new IGtAPNTemplate();
//        $apn = new IGtAPNPayload();
