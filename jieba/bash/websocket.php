...
$sk=new Sock('139.196.106.250',8000);
$sk->run();
class Sock{
	public $sockets;
	public $users;
	...
	//用户加入
	function send($k,$msg){//echo 'michelle';var_dump($k,$msg);
		parse_str($msg,$g);//so.send('nr='+esc(da)+'&key='+key);$g = array('nr'=>da,'key'=>$key);
		$ar=array();
		if($g['type']=='add'){
	...
	function send1($k,$ar,$key='all'){
		...
		if($key=='all'){
            ...
            echo "*";var_dump($k,$key);var_dump($str);echo "*";
		}
	}
