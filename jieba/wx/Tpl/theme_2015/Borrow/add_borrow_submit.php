...
<html>
    ...
    <body bgcolor="#efefef">
        <div class="headers">
            <a href="/borrow/index" style="color:white;text-decoration:none;"><img src="_STATIC_/2015/member/image/register/return.png" class="retrunImg"/></a>
            <span class="fhwz sizeOne"><a href="/borrow/index" style="color:white;text-decoration:none;">返回</a></span>
...
<!--                <span class="yf ipts sizeTwo getpassImg" style="border-left:2px solid #f7f7f7;">获取验证码</span>-->
            ...
            <input name="_borrow_mobile_" type="hidden" value="{$_borrow_mobile_}"/>
        </div>
        <script type = "text/javascript">
            $(function () {
        ...
        <script type="text/javascript">
            ...
            $('.surebtnImg').click(function () {
                ...
                $.ajax({
                    'type': 'post',
                    'dataType': 'json',
                    'url': "/borrow/add_borrow_submit",
                    "data": {'data': data},
                    success: function (json) {
                        if(json.info=="")json.info = "哎呀，网络断开了，请重新提交";
                        if (json.status == 1) {
                            location.href = '/borrow/add_borrow_success/id/' + json.data.id;
                        } else {
                            alert(json.info);
                            $('.surebtnImg').attr('src', img_dir + 'surebtn.png');
                            if (json.info == '对不起，请您重新提交')
                                location.href = '/borrow/index/';
                        }
            ...
            $('.getpassImg').click(function () {
                ...
                var token = $("input[name='_borrow_mobile_']").val();
                ...
                $.ajax({
                    'type': 'post',
                    'dataType': 'json',
                    'url': "/borrow/borrowSendSms",
                    "data": {'mobile': mobile, '_borrow_mobile_': token},
                    success: function (json) {
                        is_send = 1;
                        alert(json.info);
                        if (json.status == 1) {
