    ...
    <div class="user_info">
        ...
        <div class="acc_tips">
            ...
            <div class="info_m c_left">                
                <p>用户名：<label class="user_name"><?php echo ($_SESSION['member']['username']); ?></label></p>
                <p>密码安全度
                    <span><i style="width: <?php echo $_SESSION['member']['pwd_safety'];?>0%"></i></span>
                    <?php if($_SESSION['member']['pwd_safety'] < 4): ?>低
                    <?php elseif($_SESSION['member']['pwd_safety'] < 7): ?>
                    中
                    <?php elseif($_SESSION['member']['pwd_safety'] > 6): ?>
                    高<?php endif; ?>
                </p>               
            </div>
        </div>
        <div class="acc_user_b">
            ...
            <div class="ba_min_con">
                <p>用户名<font><input class="eidt_name_s" disabled="" type="text" name="username" value="<?php echo ($_SESSION['member']['username']); ?>"></font></p><a class="is_change" onclick="is_change(this);">修改</a>
                <p>用户ID<font><?php echo ($_SESSION['member']['id']); ?></font></p>
            </div>
        </div>
    </div>
<script type="text/javascript">
...
var change_name = function(_this)
{
        ...
        jdbox.alert(2);
        $.post("<?php echo U('/member/changeUsername');?>",{"new_name":new_name,"is_ajax":1},function(F){
            console.log(F);
