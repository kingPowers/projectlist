...
<form class="form-inline definewidth m20" action="/Member/receiveMember/" method="get">
    ...
    <a class="btn btn-primary" href="<?php echo U('Member/add_receive');?>"" >添加新的接单用户</a>
</form>
<table class="table table-bordered table-hover definewidth m10">
    ...
    <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
            ...
            <td>
               <?php if($vo['status'] == 1): ?><a href="javascript:void(0)" onclick="ban_receive(<?php echo ($vo['id']); ?>)">禁止接单</a>
               <?php else: ?>
                <a href="javascript:void(0)" onclick="allow_receive(<?php echo ($vo['id']); ?>)">允许接单</a><?php endif; ?>
            </td>
...
function ban_receive($id)
{
    ...
    $.post("<?php echo U('/Member/ban_receive');?>",{"memberid":$id,"is_ajax":1},function(F){
        console.log(F);
...
function allow_receive($id)
{
    ...
    $.post("<?php echo U('/Member/allow_receive');?>",{"memberid":$id,"is_ajax":1},function(F){
        console.log(F);
        top.jdbox.alert(F.status,F.info);
        if(F.status == 1)
        {
