...
<form class="form-inline definewidth m20" action="/Order/receiveOrderList/" method="get">
    ...
    <select name="key" class="short" id="key">
        <option>全部</option>
        <?php if(is_array($keys1)): foreach($keys1 as $key=>$vo): ?><option value='<?php echo ($key); ?>' <?php if($_GET['key']!='' && $_GET['key'] == $key): echo 'selected="selected"'; endif;?>><?php echo ($vo); ?></option><?php endforeach; endif; ?>
    </select>&nbsp;
    <span>开始时间：</span>
...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
            ...
            <td><?php echo ($vo['applyUser']); ?></td>
            <td><?php echo ($vo['applyMobile']); ?></td>
			...
			<td><?php echo ($vo['distribute_time']); ?></td>
			<td><?php echo ($vo['receive_time']); ?></td>
