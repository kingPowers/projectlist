...
<section class="content_div">
    ...
    <section id="contxt">
        ...
        <p>4.借款人应保证借款人的富友账户付款功能未受限制，并在每个还款日24:00（北京时间，下同）之前，将应还本金足额存入借款人的富友账户，且借款人同意智信创富通过富友系统在还款日自借款人的富友账户中划转应还本金至出借人富友账户和/或相应费用（如有）至智信创富富友账户。一旦借款人的富友账户付款功能受到限制，借款人应及时通知智信创富变更还款账户，以免逾期。借款人的还款时间以出借人富友账户实际收到借款人还款的时间为准，还款金额以出借人富友账户实际收到借款人的还款金额为准。</p>
        <p>5.借款人不可撤销地授权智信创富委托的富友支付机构于还款日从借款人智信创富账户中将借款人的资金划转至出借人智信创富账户；当借款人智信创富账户中的资金余额不足时，借款人不可撤销地授权富友从借款人指定的银行还款账户中代扣差额（应付款项与借款人富友账户中余额之差）。若借款人银行还款账户的余额小于前述差额，智信创富有权对借款人银行还款账户的余额进行全额扣款或视为委托还款不成功。为避免委托还款不成功，借款人应至少在还款日前一日，将资金存入借款人富友账户或指定的银行账户中；因借款人未足额存入资金导致委托还款不成功的，后果由借款人自行承担。</p>
        <p class="sp">四、逾期还款</p>
        <p>1.还款日24点前，借款人未足额支付应付款项的，则视为逾期。</p>
        <p>2.借款人逾期的，逾期罚息=剩余总本金*1%*逾期天数，自逾期之日起按本协议约定按日计收逾期罚息直至清偿完毕之日止（不含清偿完毕之日）。</p>
        ...
        <p>3.借款人和出借人提供给智信创富和富友的信息及借款人和出借人享受智信创富或富友服务产生的信息（包括本协议签署之前提供和产生的），可由智信创富和富友共享，法律禁止的除外。</p>
        ...
        <p class="sp">十一、其他</p>
        <p>1.借款人和出借人均同意并确认，借款人和出借人通过其富友账号和银行账户采取的行为所产生的法律效果和法律责任归属于借款人和出借人本身；借款人和出借人授权和委托智信创富和富友支付机构根据本协议所采取的全部行动和措施的法律后果均归属于借款人和出借人本身，与智信创富或富友无关，智信创富或富友也不因此承担任何责任。借款人和出借人同意，智信创富有权就其为借款人和出借人提供的平台服务收取服务费。</p>
...
<section class="content_div">
    ...
    <section id="contxt">
		...
		<p>3.1.1平台管理费定义：作为乙方向甲方提供的借款管理服务的报酬，甲方同意乙方成功撮合借款后向乙方支付平台管理费。平台管理费收费标准为借款总金额的{$data['rate']}%，即平台管理费总金额为：{$plat_fee}元。</p>
		...
		<p>3.2手续费收费标准及支付方式</p>
		<p>3.2.1手续费定义：作为乙方向甲方提供的第三方支付平台转账、还款服务收取的手续费，手续费收费标准为每笔成功借款支付{$fee}元。</p>
...
EOD;
	...
	static public function makenew3( $order_id,$dest='F'){
		$data = M('order_allot')
				->table("`order` o,order_allot oa")
				->field("oa.*,date_format(oa.timeadd,'%Y年%m月%d日') as timeadd,date_format(oa.back_time,'%Y年%m月%d日') as back_time,o.loanmoney,o.memberid")
				->where("o.id=oa.order_id and oa.order_id='{$order_id}'")
				->order('oa.periode')
				->select();
		$member_info = M('member_info')->where("memberid='{$data[0]['memberid']}'")->find();
		...
		$pdf->SetCreator(PDF_CREATOR);
		$pdf->SetAuthor('借吧');
		$pdf->SetTitle("分期还款协议书");
		$pdf->SetSubject("分期还款协议书");
		$pdf->SetKeywords("分期还款协议书");
		$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH, PDF_HEADER_TITLE, PDF_HEADER_STRING);
		$pdf->setFooterFont(Array(PDF_FONT_NAME_DATA, '', PDF_FONT_SIZE_DATA));
		$pdf->setHeaderFont(Array(PDF_FONT_NAME_MAIN, '', PDF_FONT_SIZE_MAIN));
		$pdf->SetDefaultMonospacedFont(PDF_FONT_MONOSPACED);
...
<header class="headers" style="display: none;">
    <h1 style="text-align:center;">分期还款协议书</h1>    
</header>
    <section class="content_div">
        <section id="contxt">
            <p class="sp jc">甲方(出借人):<font class="font_blue">李建富</font></p>
            <p class="sp jc">身份证号码:<font class="font_blue">320923198412254539</font></p>
            <p class="sp jc">乙方(借款人):<font class="font_blue">{$member_info['names']}</font></p>
            <p class="sp jc">身份证号码:<font class="font_blue">{$member_info['certiNumber']}</font></p>
            <p class="sp jc ti2">现鉴于乙方不能一次性偿还欠款。甲乙双方经过友好协商，在协商一致的基础上，达成如下还款协议：
            </p>
            <p class="ti2 sp">第一条 甲、乙双方在此确认：甲乙双方于<font class="font_blue">
            {$data['0']['timeadd']}
            </font>签订借款协议（协议编号为：<font class="font_blue">{$data['0']['order_sn']}</font>）。截止本协议签订之日乙方尚欠甲方人民币<font class="font_red">{$data['0']['loanmoney']}</font>元（以下称为剩余本金）。</p>
            <p class="ti2 sp">第二条 甲方同意乙方分4次偿还欠款及还款计划： </p>
            <p class="ti2 sp">1、甲乙双方约定第一次还款日期为：<font class="font_blue">{$data['0']['back_time']}</font>。偿还金额计算公式为25%剩余本金+剩余本金  × 0.6% × 3+3%剩余本金+25，总金额为（人民币）：<font class="font_red">{$data['0']['pay_money']}</font>元 。</p>
            <p class="ti2 sp">2、甲乙双方约定第二次还款日期为：<font class="font_blue">{$data['1']['back_time']}</font>。偿还金额计算公式为25%剩余本金+ 3%剩余本金+25，总金额为（人民币）：<font class="font_red">{$data['1']['pay_money']}</font>元 。</p>
            <p class="ti2 sp">3、甲乙双方约定第三次还款日期为：<font class="font_blue">{$data['2']['back_time']}</font>。偿还金额计算公式为25%剩余本金+ 3%剩余本金+25，总金额为（人民币）：<font class="font_red">{$data['2']['pay_money']}</font>元 。</p>
            <p class="ti2 sp">4、甲乙双方约定第四次还款日期为：<font class="font_blue">{$data['3']['back_time']}</font>。偿还金额计算公式为25%剩余本金+ 3%剩余本金+25，总金额为（人民币）：<font class="font_red">{$data['3']['pay_money']}</font>元 。</p>
            <p class="ti2 sp">第三条 乙方保证按照本协议规定时间主动偿还欠款。</p>
            <p class="ti2 sp">第四条 违约责任：如到期未还或者未还完，乙方同意按照甲乙双方签订的《借款协议》的借款总金额的1%每天计算逾期滞纳金，直至还清欠款止。乙方还需承担逾期还款带来的相应法律后果，包括但不限于出借人追讨该借款的相关差旅费、律师费、诉讼费。</p>
            <p class="ti2 sp">第五条 <font class="font_red">特别提醒</font>：在出借人、借款人确认同意本协议之前，出借人、借款人应已清楚知悉并充分理解本协议所带来的法律后果。一旦出借人、借款人在“借吧”网站或其他与智信创富相关的客户端点击接受本合同，即意味着出借人、借款人已阅读本合同所有条款，并对本合同条款的含义及相应的法律后果已全部通晓并充分理解，同意接受本合同约束。
            </p>
            <h>&nbsp;&nbsp;</h> <h>&nbsp;&nbsp;</h><h>&nbsp;&nbsp;</h>
            <p class="sp" style="padding: 30px 0;">甲方(盖章)：</p>
            <h>&nbsp;&nbsp;</h> <h>&nbsp;&nbsp;</h><h>&nbsp;&nbsp;</h>
            ...
            <p class="sp" style="float: right;text-align:right;">日期:{$data['0']['timeadd']}</p>
            </section> 
    </section>
</body>	
EOD;
		...
		$pdf->writeHTML($html, true, false, true, false, '');
		//$pdf->Image(K_PATH_IMAGES.'company-logo.png', 145, 200, 50);
		$filename = "{$data['0']['order_sn']}_4.pdf";
		$pdf->lastPage();
		$pdf->Output($filename,$dest);
		return $filename;
	}
