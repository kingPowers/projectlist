...
abstract class Action {
   ...
   /**
    ...
    protected function display($templateFile='',$charset='',$contentType='') {
    	$this->assign('pageinfo',$this->pageinfo_data);
    ...
    public function __call($method,$args) {
        if( 0 === strcasecmp($method,ACTION_NAME)) {
            ...
            if(isset($input[$args[0]])) { // 取值操作
                ...
                $data	 =	 $input[$args[0]];
               	if (is_array($data)){
               		$data = array_map_recursive('htmlspecialchars', $data);
               	}else{
	                $data = htmlspecialchars($data); // 参数过滤
               	}
                if($args[1] && $args[1]!='htmlspecialchars'){
                    $fun  =  $args[1];
                    if (is_array($data)){
                    	$data = array_map_recursive($fun, $data);
                    }else{
                    	$data = $fun($data);
                    }
                }
     ...
     * @return void
     +----------------------------------------------------------
     */
    protected function ajaxReturn($data,$info='',$status=1,$type='',$pagestring) {
        $result  =  array();
        $result['status']  =  $status;
        $result['info'] =  $info;
        $result['data'] = $data;
         $result['page'] = $pagestring;
