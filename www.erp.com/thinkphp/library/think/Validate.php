    ...
    protected function checkItem($field, $value, $rules, $data, $title = '', $msg = [])
    {
        ...
        foreach ($rules as $key => $rule) {
            if ($rule instanceof \Closure) {
                ...
                if (0 === strpos($info, 'require') || (!is_null($value) && '' !== $value) || (0 === stripos(strtolower($rule),'modelcustom'))) {
                    // 验证类型                   
                    
                    if (false !== stripos(strtolower($rule),'modelcustom')) {
                        $modelName = explode("-",$rule)[1];
                        $callback = [new $modelName(),$type];
                    } else {
